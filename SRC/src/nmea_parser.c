...
#define CONFIG_NMEA_PARSER_RING_BUFFER_SIZE 512
...
static void esp_handle_uart_pattern(esp_gps_t *esp_gps) {
    ...
    if (pos != -1) {
        ...
        int read_len =
            uart_read_bytes(esp_gps->uart_port, esp_gps->buffer, pos + 1, 100 / portTICK_PERIOD_MS);
...
esp_err_t nmea_parser_add_handler(nmea_parser_handle_t nmea_hdl,
    ...
    return esp_event_handler_register_with(
        esp_gps->event_loop_hdl, ESP_NMEA_EVENT, ESP_EVENT_ANY_ID, event_handler, handler_args);
}
